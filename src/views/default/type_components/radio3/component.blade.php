<div class='form-group {{$header_group_class}} {{ ($errors->first($name))?"has-error":"" }}' id='form-group-{{$name}}' style="{{@$form['style']}}">
    <label class='control-label col-sm-2'>{{$form['label']}} (<font color="blue">M</font> | <font color="red">F</font>)
        @if($required)
    ...
    <div class="{{$col_width?:'col-sm-10'}}">
        ...
        @if($form['dataenum']!='')
			...
			@foreach($dataenum as $k=>$d)
                ...
                $checked1 = ($value && in_array($val, $value)) ? "active" : "";
				$checked = ($value && in_array($val, $value)) ? "checked" : "";
				$stylich = ($compt == '0'  ? "btn-primary" : "btn-danger");
				$glyph = ($compt == '0'  ? "fa-mars" : "fa-venus");
				
				$compt='1';
                ?>
					
				<label class="btn {{$stylich}} {{$checked1}}">
				<input type="radio"  autocomplete="off" {{$disabled}} {{$checked}} name="{{$name}}" value="{{$val}}" >
