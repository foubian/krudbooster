    ...
    <div class="{{ $col_width ?: 'col-sm-10' }}">
        ...
        @if ($form['dataenum'] != '')
            ...
            @foreach ($dataenum as $k => $d)
                ...
                ?>
                <div class="checkbox-wrapper-10 {{ $disabled }}">
                    <input type="checkbox" class="tgl tgl-flip" {{$disabled}} {{$checked}} id="{{$name}}{{$val}}" name="{{$name}}" value="{{$val}}">
                    <label for="{{$name}}{{$val}}" data-tg-on="On" data-tg-off="Off" class="tgl-btn"></label>
                </div>
            @endforeach
        @endif
