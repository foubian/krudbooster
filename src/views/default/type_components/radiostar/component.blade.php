<div class='form-group {{ $header_group_class }} {{ $errors->first($name) ? 'has-error' : '' }}'
    ...
    <div class="{{ $col_width ?: 'col-sm-10' }}">
        ...
        @if ($form['dataenum'] != '')
            ...
            ?>
            <div class="radio-toolbar {{ $disabled }}">
                @foreach ($dataenum as $k => $d)
                    ...
                    ?>
                    <input type="radio" {{ $disabled }} {{ $checked }} id="{{ $name }}{{ $val }}"
                        name="{{ $name }}" value="{{ $val }}">
                    <label class="{{ $klass }}" for="{{ $name }}{{ $val }}">{{ $label }}</label>
                @endforeach
            </div>
        @endif
                ...
                echo "
                                											<div data-val='$val' class='input-radio-wrapper $disabled'>
                                											  <label class='radio-inline'>
                                											    <input type='radio' $disabled $checked name='" .
                    $name .
                    "' value='" .
                    $d->id .
                    "'> " .
