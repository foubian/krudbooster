                        ...
                        $('#{{ $name }}').select2({
                            ...
                            ajax: {
                                ...
                                data: function(params) {
                                    var value = $("input[type=radio][name={{ $form['radioparent'] }}]:checked")
                                        .val();
                                    var whir = "{!! addslashes($where) !!}";
                                    if (value) {
                                        whir = "type_com =" + value;
                                    }
                                    var query = {
...
@else
    @push('bottom')
        <script type="text/javascript">
            ...
            function task() {
                var newStateVal = "fffffff";
                // Set the value, creating a new option if necessary
                if ($("#{{ $name }}").find("option[value=" + newStateVal + "]").length) {
                    $("#{{ $name }}").val(newStateVal).trigger("change");
                } else {
                    // Create the DOM option that is pre-selected by default
                    var newState = new Option(newStateVal, newStateVal, true, true);
                    // Append it to the select
                    $("#{{ $name }}").append(newState).trigger('change');
                    $('#{{ $name }}').select2('close');
                }
            }
        </script>
