...
<form id='form-table' method='post' action='{{ CRUDBooster::mainpath('action-selected') }}'>
    ...
    <div class="col-sm-12">
        <table id='table_dashboard' class="table table-bordered table-striped dataTable">
            <thead>
                <tr class="active">
                    ...
                    @if ($button_table_action)
                        @if (CRUDBooster::isUpdate() || CRUDBooster::isDelete() || CRUDBooster::isRead())
                            <th width='{{ isset($button_action_width) ? $button_action_width : 'auto' }}' style="text-align:center">{{ cbLang('action_label') }}</th>
                        @endif
                    @endif
                </tr>
            </thead>
            <tbody>
                ...
                @foreach ($html_contents['html'] as $i => $hc)
                    ...
                    @foreach ($hc as $j => $h)
                        <td style="vertical-align: middle;max-width: 150px;{{ $columns[$j-1]['style'] }}">{!! $h !!}</td>
                    @endforeach
                    </tr>
                @endforeach
            </tbody>
                        ...
                        <div class="modal-body">
                            ...
                            <div class='form-group'>
                                ...
                                <div class='row-filter-combo row'>
                                    ...
                                    <div class='col-sm-5'>
                                        <input type='text' class='filter-value form-control' style="{{ CRUDBooster::getTypeFilter($col['field_with']) == 'between' ? 'display:none' : 'display:block' }}" disabled
                                        ...
                                        <div class='row between-group' style="{{ CRUDBooster::getTypeFilter($col['field_with']) == 'between' ? 'display:block' : 'display:none' }}">
                                            <div class='col-sm-6'>
                                                <div class='input-group {{ $col['type_data'] == 'time' ? 'bootstrap-timepicker' : '' }}'>
                                                    ...
                                                    <input {{ CRUDBooster::getTypeFilter($col['field_with']) != 'between' ? 'disabled' : '' }} type='text'
                    ...
                    <form method='post' target="_blank" action='{{ CRUDBooster::mainpath('export-data?t=' . time()) }}'>
                        ...
                        <div class="modal-body">
                            ...
                            <p><a href='javascript:void(0)' class='toggle_advanced_report'><i class='fa fa-plus-square-o'></i>
                                    {{ cbLang('export_dialog_show_advanced') }}</a></p>
                            ...
                            <div id='advanced_export' style='display: none'>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class='form-control' name='page_size'>
                                        ...
                                        <option <?= $setting->default_paper_size == 'Legal' ? 'selected' : '' ?> value='Legal'>
                                            Legal</option>
                                        <option <?= $setting->default_paper_size == 'Ledger' ? 'selected' : '' ?> value='Ledger'>Ledger</option>
                                        <?php for($i = 0;$i <= 8;$i++):$select = ($setting->default_paper_size == 'A'.$i) ? "selected" : "";
?>
