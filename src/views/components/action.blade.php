@foreach ($addaction as $a)
    ...
    echo \"<a class='btn btn-xs btn-\$color' title='\$title' onclick='\$confirm_box' href='\$url' target='\$target'><i class='\$icon'></i> $label</a>&nbsp;\";
        ...
        echo "<a class='btn btn-xs btn-$color' title='$title' onclick='$confirm_box' href='$url' target='$target'> <span class='spin-icon'><i class='$icon'></i> $label</span></a>&nbsp;";
    }
    ?>
@endforeach
...
@elseif($button_action_style == 'button_icon_text')
    @if (CRUDBooster::isRead() && $button_detail)
        <a class='btn btn-xs btn-primary btn-detail' title='{{ cbLang('action_detail_data') }}'
            ...
            <i class='fa fa-eye'></i> {{ cbLang('action_detail_data') }}</a>
    @endif
        ...
        <ul class='dropdown-menu dropdown-menu-action' role='menu'>
            @foreach ($addaction as $a)
                ...
                if (isset($a['showIf'])) {
                    ...
                    echo "<li><a title='$title' href='$url'><i class='$icon'></i> $label</a></li>";
                }
                ?>
            @endforeach
            ...
            @if (CRUDBooster::isRead() && $button_detail)
                <li><a class='btn-detail' title='{{ cbLang('action_detail_data') }}'
                        href='{{ CRUDBooster::mainpath('detail/' . $row->$pk) . '?return_url=' . urlencode(Request::fullUrl()) }}'><i
                            class='fa fa-eye'></i>
                        {{ cbLang('action_detail_data') }}</a></li>
            @endif
...
@else
    @if (CRUDBooster::isRead() && $button_detail)
        <a class='btn btn-xs spin-icon outline btn-primary btn-detail' title='{{ cbLang('action_detail_data') }}'
            href='{{ CRUDBooster::mainpath('detail/' . $row->$pk) . '?return_url=' . urlencode(Request::fullUrl()) }}'>
    ...
    @if (CRUDBooster::isUpdate() && $button_edit)
        <a class='btn btn-xs outline spin-icon btn-success btn-edit' title='{{ cbLang('action_edit_data') }}'
    ...
    @if (CRUDBooster::isDelete() && $button_delete)
        <?php $url = CRUDBooster::mainpath('delete/' . $row->$pk); ?>
        <a class='btn btn-xs spin-icon outline btn-warning btn-delete' title='{{ cbLang('action_delete_data') }}'
            href='javascript:;' onclick='{{ CRUDBooster::deleteConfirm($url) }}'>
