...
class CRUDBooster
{
    ...
    {
        $role = DB::table('cms_users')->where('id', Session::get('admin_id'))->first();
        return $role->id_cms_privileges;
    }
    public static function myPedagojie()
    {
        $month = date('m');
        if ($month > 8) {
            $year = date('Y');
        ...
        $pedagog = DB::table('infopedagog')
            ->where('student_id', Session::get('admin_id'))
            ->where('anne_peda', $year)
            ->first();
        return $pedagog;
    }
    //
    ...
    public static function myFyl()
    {
        return Session::get('fyl');
    }
    public static function myNivo()
    {
        return Session::get('nivo');
    }
    ...
    public static function langConfirm($redirectTo)
    {
        echo "Swal.fire({
				title: \"" . cbLang('delete_title_confirm') . "\",
				text: \"" . cbLang('language_description_confirm') . "\",
				icon: \"warning\",
				showCancelButton: true,
				confirmButtonColor: \"#ff0000\",
				confirmButtonText: \"" . cbLang('confirmation_yes') . "\",
    ...
    public static function insertLogg($idrecord,$description,$icon,$color,$operation)
    {
        if (CRUDBooster::getSetting('api_debug_mode')) {
            $a                 = [];
            $a['user'] = self::myId();
            ...
            $a['icon']    = $icon;
            $a['color']    = $color;
            $a['module']      = CRUDBooster::getModulePath();
            $a['operation']    = $operation;
            $log = Loggs::create($a);

            //DB::table('cms_loggs')->insert($a);
        }
    }
