<?php
namespace foubian\krudbooster\controllers;

error_reporting(E_ALL ^ E_NOTICE);

use CB;
use foubian\krudbooster\export\DefaultExportXls;
use CRUDBooster;
...
class CBController extends Controller
{
    ...
    public function cbLoader()
    {
        ...
        $this->data['is_child']              = $this->is_child;
    ...
    public function getIndex()
    {
        ...
        foreach ($data['result'] as $row) {
            ...
            foreach ($columns_table as $col) {
                ...
                if (isset($col['image'])) {
                    if ($value == '') {
                        $value = "<a  data-lightbox='roadtrip' rel='group_{{$table}}' title='$label: $title' href='" . asset('vendor/crudbooster/avatar.jpg') . "'><div class='hex'><img width='40px' height='40px' src='" . asset('vendor/crudbooster/avatar.jpg') . "'/></div></a>";
                    } else {
                        $pic   = (strpos($value, 'http://') !== false) ? $value : asset($value);
                        $value = "<a data-lightbox='roadtrip'  rel='group_{{$table}}' title='$label: $title' href='" . $pic . "'><div class='hex'><img  src='" . $pic . "'/></div></a>";
                    }
                }
                ...
                if (@$col['numerik']) {
                    if ($value) {
                        ...
                        $value = '<a class="btn btn-xs spin-icon outline ' . @$col['ncolor'] . '">
                                    <span> ' . $value . ' ' . @$col['koi'] . '</span></a>';
                        /*  $value ='<a class="btn btn-xs spin-icon outline '. @$col[ 'ncolor' ].' btn-detail"   href="#">
                    <span><i class="fa fa-eye"></i>'. $value.' '. @$col[ 'koi' ].'</span>
                    </a>';*/
                    } else {
                        $value = " - ";
                    }
            ...
            if ($this->button_table_action):
                ...
                $html_content[]      = "<div class='button_action pull-" . cblang('left') . "' >" . view('crudbooster::components.action', compact('addaction', 'row', 'button_action_style', 'parent_field'))->render() . "</div>";
    ...
    public function input_assignment($id = null)
    {
        ...
        foreach ($this->data_inputan as $ro) {
            ...
            if ($ro['type'] == 'chekregion' || $ro['type'] == 'chekit') {
            ...
            if (@$ro['type'] == 'upload' || $ro['type'] == 'uploadcustom') {
    ...
    public function getAdd()
    {
       // dd($param);
        ...
        if (in_array(App::getLocale(), ['ar', 'fa'])) {
            $page_title = cbLang("add_data_page_title", ['module' => CRUDBooster::getCurrentModule()->name_ar]);
        } else {
    ...
    {   
        ...
        $page_menu = Route::getCurrentRoute()->getActionName();
        if (in_array(App::getLocale(), ['ar', 'fa'])) {
            $page_title = cbLang("edit_data_page_title", ['module' => CRUDBooster::getCurrentModule()->name_ar, 'name' => $row->{$this->title_field}]);
        } else {
    ...
    public function getDetail($id)
    {
        ...
        $page_menu = Route::getCurrentRoute()->getActionName();
        if (in_array(App::getLocale(), ['ar', 'fa'])) {
            $page_title = cbLang("detail_data_page_title", ['module' => $module->name_ar, 'name' => $row->{$this->title_field}]);
        } else {
